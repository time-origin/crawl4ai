...
在项目根目录 (`crawl4ai`) 下打开终端，并执行以下命令来构建镜像。镜像名称将直接设置为阿里云仓库地址，标签为 `latest`。

```sh
docker build -t registry.cn-shanghai.aliyuncs.com/dazzrepo/custom-crawl4ai:latest -f Dockerfile-custom .
```
...
**步骤 2.1: 登录阿里云 Registry**
```sh
# 使用您的阿里云账户全名和密码登录
docker login --username=18616786327@163.com registry.cn-shanghai.aliyuncs.com
```

**步骤 2.2: 推送镜像**

由于上一步构建时已正确命名，直接推送即可。
```sh
docker push registry.cn-shanghai.aliyuncs.com/dazzrepo/custom-crawl4ai:latest
```

### 3. 执行爬虫

你可以使用 `docker run` 命令来启动一个容器并执行爬虫任务。

**示例 1: 基本执行**
```sh
docker run --rm -it registry.cn-shanghai.aliyuncs.com/dazzrepo/custom-crawl4ai:latest python -m crawl4ai.crawlers.x_com.production_crawler --original-task-id 101 --keyword "OpenAI"
```

**示例 2: 带全参数执行**
```sh
docker run --rm -it registry.cn-shanghai.aliyuncs.com/dazzrepo/custom-crawl4ai:latest python -m crawl4ai.crawlers.x_com.production_crawler \
    --original-task-id 123 \
...
然后执行：
```shell
./production_crawler_linux --original-task-id 101 --keyword "generative AI" --scan-scrolls 3
```
